...
function defineImageHeight() {
    var nameHeight = $('.person-info').height() + 100;
    if(($(window).width() > 768) && ($(window).height() < (($(window).width() - 150) / 2 + nameHeight))) {
        $('.right').addClass('auto-heigt');
    } else {
        $('.right').removeClass('auto-heigt');
    }
}
